...
/**
 * Desktop (JVM) implementation of CcgonowdepKMP
 *
 * This implementation uses JNI to bridge to the native C++ library on desktop platforms
 * (Windows, macOS, Linux).
 *
 * Note: The native library (libccgonowdep.so/.dylib/.dll) should be available
...
actual class CcgonowdepKMP actual constructor() {
    ...
    actual fun setDebugLog(enable: Boolean) {
        if (!nativeLibraryLoaded) {
            println("CcgonowdepKMP.setDebugLog (stub): $enable")
            return
        }
