...
This document explains the architectural decision to use **Kotlin/Native** targets instead of **JVM** targets for desktop platforms (macOS and Linux) in the Ccgonowdep KMP library.
...
```kotlin
// desktopMain/kotlin/CcgonowdepKMP.desktop.kt
actual class CcgonowdepKMP actual constructor() {
    companion object {
        init {
            System.loadLibrary("ccgonowdep")  // Load JNI library
        }
    }
...
#### Native Desktop (New)
```kotlin
// nativeMain/kotlin/CcgonowdepKMP.native.kt
@OptIn(ExperimentalForeignApi::class)
actual class CcgonowdepKMP actual constructor() {
    actual fun setDebugLog(enable: Boolean) {
        // Direct C function call through cinterop
        ccgonowdep_set_debug_log(if (enable) 1 else 0)
    }
}
```
...
│   └── kotlin/
│       └── CcgonowdepKMP.desktop.kt (JNI implementation)
│
├── nativeMain/          # Shared Native implementation (macOS, Linux)
│   └── kotlin/
│       └── CcgonowdepKMP.native.kt (cinterop implementation)
│
...
**Before**:
- `ccgonowdep-kmp-desktop` (single artifact for all desktop platforms)

**After**:
- `ccgonowdep-kmp-desktop` (Windows JVM)
- `ccgonowdep-kmp-macos` (macOS Native)
- `ccgonowdep-kmp-linux` (Linux Native)
...
**File**: `src/nativeInterop/cinterop/Ccgonowdep.def`
