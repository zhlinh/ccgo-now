...
function archiveOhosArtifacts(rootDir: string): void {
    ...
    const archiveName = `ARCHIVE_${projectNameUpper}_OHOS_SDK-${fullVersion}`;
    ...
    // Find and copy HAR file (to both targetDir and archiveDir)
    ...
    if (harFiles.length === 0) {
        ...
        // Copy to targetDir for standalone HAR access
        const harDest = path.join(targetDir, harFilename);
        fs.copyFileSync(harFile, harDest);
        console.log(`Copied HAR: ${harDest}`);
        // Also copy to archiveDir so it's included in the ZIP
        const harArchiveDest = path.join(archiveDir, harFilename);
        fs.copyFileSync(harFile, harArchiveDest);
        console.log(`Added HAR to archive: ${harArchiveDest}`);
    }
