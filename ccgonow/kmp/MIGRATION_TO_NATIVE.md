...
This document explains the architectural decision to use **Kotlin/Native** targets instead of **JVM** targets for desktop platforms (macOS and Linux) in the Ccgonow KMP library.
...
```kotlin
// desktopMain/kotlin/CcgonowKMP.desktop.kt
actual class CcgonowKMP actual constructor() {
    companion object {
        init {
            System.loadLibrary("ccgonow")  // Load JNI library
        }
    }

    actual fun setDebugLog(enable: Boolean) {
        // Call via reflection to JNI class
        val jniClass = Class.forName("com.mojeter.ccgo.samples.ccgonow.jni.CcgonowJni")
...
#### Native Desktop (New)
```kotlin
// nativeMain/kotlin/CcgonowKMP.native.kt
@OptIn(ExperimentalForeignApi::class)
actual class CcgonowKMP actual constructor() {
    actual fun setDebugLog(enable: Boolean) {
        // Direct C function call through cinterop
        ccgonow_set_debug_log(if (enable) 1 else 0)
    }
}
```
...
│   └── kotlin/
│       └── CcgonowKMP.desktop.kt (JNI implementation)
│
├── nativeMain/          # Shared Native implementation (macOS, Linux)
│   └── kotlin/
│       └── CcgonowKMP.native.kt (cinterop implementation)
│
...
**Before**:
- `ccgonow-kmp-desktop` (single artifact for all desktop platforms)

**After**:
- `ccgonow-kmp-desktop` (Windows JVM)
- `ccgonow-kmp-macos` (macOS Native)
- `ccgonow-kmp-linux` (Linux Native)
...
**File**: `src/nativeInterop/cinterop/Ccgonow.def`
